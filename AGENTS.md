...
O AIOX usa um modelo de 4 camadas (L1-L4) para separar artefatos do framework e do projeto. Deny rules em `.Codex/settings.json` reforçam isso deterministicamente.
...
> **Referência formal:** `.Codex/settings.json` (deny/allow rules), `.Codex/rules/agent-authority.md`
<!-- AIOX-MANAGED-END: framework-boundary -->

<!-- AIOX-MANAGED-START: rules-system -->
## Rules System

O AIOX carrega regras contextuais de `.Codex/rules/` automaticamente. Regras com frontmatter `paths:` só carregam quando arquivos correspondentes são editados.
...
| `agent-memory-imports.md` | Agent memory lifecycle and AGENTS.md ownership |
...
> **Diretório:** `.Codex/rules/` — rules são carregadas automaticamente pelo Codex quando relevantes.
...
- `npm run pos-push` - Verify post-push sync/status and suggest next DevOps actions
...
---
*Synkra AIOX Codex Configuration v2.0*
