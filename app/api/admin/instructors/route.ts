...
import { listInstructors, normalizeCatalogListParams } from "@/lib/supabase/admin-catalog-read-models";

/**
 * Read model administrativo de INSTRUTORES (REC-304 — continua FND-08).
 *
 ...
 * paginação e busca por nome retornam dados autorizados. O HMAC de produção
...
async function handleGet(request: Request) {
  ...
  try {
    ...
    const result = await listInstructors(guard.adminClient, params);
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    logger.error("api/admin/instructors.list error", {
      err: error,
      route: "api/admin/instructors",
    });
    return NextResponse.json({ ok: false, error: "Erro ao listar instrutores." }, { status: 500 });
  }
}
