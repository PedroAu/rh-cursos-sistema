        ...
        return (
          <div key={label} className="flex flex-1 items-start">
            ...
            {index < steps.length - 1 ? (
              <div className={cn("mt-[19px] h-0.5 flex-1", index < currentStep ? "bg-tk-brand" : "bg-tk-line")} />
            ) : null}
          </div>
        );
      })}
...
export function CourseCheckoutPage() {
  ...
  useEffect(() => {
    ...
    setForm((current) => (current.classId === defaultClassId ? current : { ...current, classId: defaultClassId }));
  ...
  const discountValue = hasDiscount ? Math.round((course?.price ?? 0) * 0.1 * 100) / 100 : 0;
  const total = Math.max((course?.price ?? 0) - discountValue, 0);
  ...
  const validateStep = () => {
    ...
    if (step === 0) {
      ...
      if (form.buyerType === "pf") {
        ...
        if (!form.organization.trim()) nextErrors.organization = "Informe a razão social.";
        if (form.cnpj.replace(/\D/g, "").length !== 14) nextErrors.cnpj = "CNPJ deve ter 14 dígitos.";
      ...
      if (form.phone.replace(/\D/g, "").length < 10) nextErrors.phone = "Telefone deve ter no mínimo 10 dígitos.";
      if (form.cpf.replace(/\D/g, "").length !== 11) nextErrors.cpf = "CPF deve ter 11 dígitos.";
    }
  ...
  const finish = async () => {
    if (!validateStep() || !course || !effectiveClassId) return;
      ...
      <section className="page-section">
        <div className="container">
          <EmptyState
            ...
            description="Verifique o link do checkout ou volte para o catálogo."
            ...
            <div className="mx-auto max-w-[1100px]">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm text-tk-ink-muted">{course.title}</p>
                </div>
              ...
              {step === 0 ? (
                <>
                  <section className="rounded-tk-card border border-tk-line bg-white p-6 shadow-tk-glass md:p-8">
                    ...
                    <div aria-label="Escolha a turma" className="grid gap-2" role="radiogroup">
                      {courseClasses.map((trainingClass) => {
                        ...
                        const vacancyLabel = trainingClass.availableSeats <= 3 ? "Últimas vagas" : "Abertas";
                          ...
                          >
                            ...
                            <span
                              className={cn(
                                ...
                                trainingClass.availableSeats <= 3 ? "bg-[#fdecee] text-tk-error" : "bg-[#e6f4ef] text-tk-success",
                              )}
                            >
                              {vacancyLabel}
                            </span>
                  ...
                  <section className="rounded-tk-card border border-tk-line bg-white p-6 shadow-tk-glass md:p-8">
                      ...
                      <div className="mb-5 grid gap-4 md:grid-cols-2">
                        <div className="md:col-span-2">
                          <Field label="Nome completo" required error={fieldErrors.studentName}>
                            <Input value={form.studentName} onChange={(event) => updateField("studentName", event.target.value)} placeholder="Seu nome completo" />
                          </Field>
                        </div>
                        <Field label="CPF" required error={fieldErrors.cpf}>
                          <Input value={form.cpf} onChange={(event) => updateField("cpf", formatCPF(event.target.value))} placeholder="000.000.000-00" />
                        </Field>
                        <Field label="Telefone" required error={fieldErrors.phone}>
                          <Input type="tel" value={form.phone} onChange={(event) => updateField("phone", formatPhone(event.target.value))} placeholder="(00) 00000-0000" />
                        </Field>
                        <div className="md:col-span-2">
                          <Field label="E-mail" required error={fieldErrors.email}>
                            <Input type="email" value={form.email} onChange={(event) => updateField("email", event.target.value)} placeholder="voce@email.com.br" />
                          </Field>
                        </div>
                      </div>
                    ) : (
                      <div className="mb-5 grid gap-4 md:grid-cols-2">
                        <div className="md:col-span-2">
                          <Field label="Razão social" required error={fieldErrors.organization}>
                            <Input value={form.organization} onChange={(event) => updateField("organization", event.target.value)} placeholder="Nome da empresa" />
                          </Field>
                        </div>
                        <Field label="CNPJ" required error={fieldErrors.cnpj}>
                          <Input value={form.cnpj} onChange={(event) => updateField("cnpj", formatCNPJ(event.target.value))} placeholder="00.000.000/0000-00" />
                        </Field>
                        <Field label="Nome do responsável" required error={fieldErrors.contactName}>
                          <Input value={form.contactName} onChange={(event) => updateField("contactName", event.target.value)} placeholder="Quem vai participar" />
                        </Field>
                        <Field label="Telefone" required error={fieldErrors.phone}>
                          <Input type="tel" value={form.phone} onChange={(event) => updateField("phone", formatPhone(event.target.value))} placeholder="(00) 00000-0000" />
                        </Field>
                        <Field label="E-mail" required error={fieldErrors.email}>
                          <Input type="email" value={form.email} onChange={(event) => updateField("email", event.target.value)} placeholder="financeiro@empresa.com.br" />
                        </Field>
                        <div className="md:col-span-2">
                    ...
                    <div className="mb-4 border-t border-tk-line pt-4">
                      <p className="mb-3 text-caption font-semibold uppercase tracking-[0.06em] text-tk-ink-muted">Endereço de cobrança</p>
                      <div className="grid gap-4 md:grid-cols-3">
                        <Field label="CEP">
                          <Input value={form.zipCode} onChange={(event) => updateField("zipCode", formatCEP(event.target.value))} placeholder="00000-000" />
                        </Field>
                        <div className="md:col-span-2">
                          <Field label="Endereço">
                            <Input value={form.street} onChange={(event) => updateField("street", event.target.value)} placeholder="Rua, avenida..." />
                          </Field>
                        </div>
                        <Field label="Número">
                          <Input value={form.streetNumber} onChange={(event) => updateField("streetNumber", event.target.value)} placeholder="123" />
                        </Field>
                        <Field label="Complemento">
                          <Input value={form.complement} onChange={(event) => updateField("complement", event.target.value)} placeholder="Sala, bloco..." />
                        </Field>
                        <Field label="Bairro">
                          <Input value={form.neighborhood} onChange={(event) => updateField("neighborhood", event.target.value)} placeholder="Bairro" />
                        </Field>
                        <Field label="Cidade">
                          <Input value={form.city} onChange={(event) => updateField("city", event.target.value)} placeholder="Cidade" />
                        </Field>
                        <Field label="Estado">
                          <select
                            aria-label="Estado"
                ...
                <section className="rounded-tk-card border border-tk-line bg-white p-6 shadow-tk-glass md:p-8">
                  ...
                  <div className="grid gap-2" role="radiogroup" aria-label="Forma de pagamento">
                    <PaymentCard checked={form.paymentMethod === "Cartão"} title="Cartão de crédito" description="Em até 6x sem juros" icon={<CreditCard className="h-4 w-4 text-tk-brand" aria-hidden="true" />} onClick={() => updateField("paymentMethod", "Cartão")} />
                    <PaymentCard checked={form.paymentMethod === "Pix"} title="Pix" description="Pagamento à vista com confirmação rápida" icon={<QrCode className="h-4 w-4 text-tk-brand" aria-hidden="true" />} onClick={() => updateField("paymentMethod", "Pix")} />
                    <PaymentCard checked={form.paymentMethod === "Boleto"} title="Boleto bancário" description="Vencimento em 3 dias úteis" icon={<TicketPercent className="h-4 w-4 text-tk-brand" aria-hidden="true" />} onClick={() => updateField("paymentMethod", "Boleto")} />
                    <PaymentCard checked={form.paymentMethod === "Empenho"} title="Nota de empenho" description="Para órgãos públicos, sem pagamento online" icon={<Landmark className="h-4 w-4 text-tk-brand" aria-hidden="true" />} onClick={() => updateField("paymentMethod", "Empenho")} />
                  </div>
                    ...
                    <div className="mt-5 grid gap-4 rounded-[14px] border border-[#ead9b0] bg-[var(--tk-cream)] p-5 md:grid-cols-2">
                      <div className="md:col-span-2">
                        <Field label="Nome impresso no cartão" required error={fieldErrors.cardName}>
                          <Input value={form.cardName} onChange={(event) => updateField("cardName", event.target.value)} placeholder="Como está no cartão" />
                        </Field>
                      </div>
                      <div className="md:col-span-2">
                        <Field label="Número do cartão" required error={fieldErrors.cardNumber}>
                          <Input value={form.cardNumber} onChange={(event) => updateField("cardNumber", event.target.value)} placeholder="0000 0000 0000 0000" />
                        </Field>
                      </div>
                      <Field label="Validade" required error={fieldErrors.cardExpiry}>
                        <Input value={form.cardExpiry} onChange={(event) => updateField("cardExpiry", event.target.value.replace(/[^\d/]/g, "").slice(0, 5))} placeholder="MM/AA" />
                      </Field>
                      <Field label="CVV" required error={fieldErrors.cardCvv}>
                        <Input value={form.cardCvv} onChange={(event) => updateField("cardCvv", event.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="123" />
                      </Field>
                      <div className="md:col-span-2">
                  ...
                  {form.paymentMethod === "Pix" || form.paymentMethod === "Boleto" ? (
                    <div className="mt-5 flex gap-3 rounded-[14px] border border-[#ead9b0] bg-[var(--tk-cream)] p-5 text-sm leading-6 text-tk-ink-muted">
                      ...
                      <p>O código de pagamento é gerado assim que você finalizar a compra. A vaga é confirmada após a identificação do pagamento.</p>
                    </div>
                  ) : null}
                  ...
                  {form.paymentMethod === "Empenho" ? (
                    <div className="mt-5 grid gap-4 rounded-[14px] border border-[#ead9b0] bg-[var(--tk-cream)] p-5 md:grid-cols-2">
                      <div className="md:col-span-2 flex gap-3 text-sm leading-6 text-tk-ink-muted">
                        ...
                        <p>Para órgãos públicos, a vaga é reservada mediante nota de empenho. Nossa equipe envia a documentação após a finalização.</p>
                      </div>
                      <div className="md:col-span-2">
                        <Field label="Órgão / entidade" required error={fieldErrors.commitmentUnit}>
                          <Input value={form.commitmentUnit} onChange={(event) => updateField("commitmentUnit", event.target.value)} placeholder="Nome do órgão" />
                        </Field>
                      </div>
                      <Field label="Código UASG (opcional)">
                        <Input value={form.commitmentCode} onChange={(event) => updateField("commitmentCode", event.target.value)} placeholder="000000" />
                      </Field>
                    </div>
                  ) : null}
                  ...
                  <div className="mt-6 border-t border-tk-line pt-4">
                    <label className="flex items-start gap-3 text-sm text-tk-ink">
                      <Checkbox checked={form.acceptedTerms} onCheckedChange={(checked) => updateField("acceptedTerms", checked)} aria-label="Li e aceito os termos de uso e a política de cancelamento" />
            ...
            <aside className="grid gap-4 md:sticky md:top-6 md:self-start">
              <div className="overflow-hidden rounded-tk-card border border-tk-line bg-white shadow-card">
                <div className="p-6">
                  ...
                  <p className="mb-1 text-caption font-semibold uppercase tracking-[0.06em] text-tk-ink-muted">Você está comprando</p>
                  ...
                  <div className="mt-5 grid gap-2 border-t border-tk-line pt-4 text-sm text-tk-ink-muted">
                    ...
                    <div className="flex items-center justify-between border-t border-tk-line pt-3">
                      ...
                      <span className="font-display text-2xl font-bold text-tk-brand">{formatCurrency(total)}</span>
                    </div>
                    <div className="text-caption">
                      {form.paymentMethod === "Cartão"
                        ? `em ${installments}x de ${formatCurrency(total / installments)} sem juros`
                        : form.paymentMethod === "Empenho"
                  ...
                  {couponStatus === "invalid" ? <p className="mt-2 text-caption text-tk-error">Cupom inválido ou expirado.</p> : null}
                  {couponStatus === "valid" ? <p className="mt-2 text-caption text-tk-success">Cupom aplicado com sucesso!</p> : null}
                </div>
              </div>
              ...
              <div className="flex gap-3 rounded-2xl border border-tk-line bg-white px-5 py-4 text-sm leading-6 text-tk-ink-muted">
                ...
                <p>
                  <strong className="text-tk-ink">Compra 100% segura.</strong> Seus dados são protegidos e o pagamento é processado com criptografia de ponta a ponta.
                </p>
              </div>
