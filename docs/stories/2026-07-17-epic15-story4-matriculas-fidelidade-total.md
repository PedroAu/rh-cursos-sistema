...
## Acceptance Criteria
- [ ] Tela usa o título “Matrículas”, resumo dinâmico e filtros coerentes com `Admin — Matrículas`.
- [ ] Tabela apresenta Aluno, Turma, Inscrição, Pagamento, Valor e Ações usando dados reais disponíveis.
- [ ] Nenhum estado financeiro é inventado; ausências aparecem como informação indisponível.
- [ ] CRUD/atualização de status, validação e exportação existentes permanecem funcionais.
- [ ] Testes cobrem renderização, filtro, status, empty state e responsividade.
