...
- `src/lib/supabase/rh-cursos-api.ts` (modificado — `fetchPublicCatalogFromSupabaseServer()` memoizado por request com `cache()` para evitar round-trips duplicados em rotas públicas dinâmicas)
