...
Fechamento atualizado em 2026-07-03:
- `npm run test:e2e:smoke` passou dentro do fluxo `npm test`
- `npm test` passou completo (`134 passed`)
- `npm audit --audit-level=high` ficou sem findings `high`/`critical` após `overrides` transitivos para `tmp`, `uuid` e `js-yaml`
- `tests/visual.baseline.spec.ts` foi alinhado à UI atual do `/blog` para remover falso negativo do gate visual

Com isso, a story sai de `InProgress` e volta para `InReview` com ACs completos.
