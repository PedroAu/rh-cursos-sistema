...
**Status:** **Encerrada (G5 CONCERNS, 2026-07-19)** — [REC-502](../stories/2026-07-19-rec-502-encerrar-incidente-post-mortem.md) consolidou G0–G4 e declarou o encerramento formal do incidente, com 13 ações preventivas registradas (owner: Pedro Augusto) e waiver explícito de REC-005. CONCERNS, não PASS puro, porque três itens de G4 (E2E com fixtures reais, homologação formal, smoke pós-deploy) exigem infraestrutura de produção não disponível na sessão de fechamento — viraram checklist obrigatório antes do próximo deploy real (ver REC-502 §4, item 13). Histórico anterior preservado abaixo. Ondas 0, 1 e 2 quase completas; Onda 3 iniciada (ADR). `Done` (gate @qa PASS/CONCERNS não-bloqueante): REC-001, REC-403, REC-302, REC-401, REC-402, REC-301, REC-002 (CONCERNS 80/100, follow-up SEC-104), REC-003 (PASS 90/100), REC-004 (PASS 95/100), REC-101 (CONCERNS 82/100, follow-up SEC-107), REC-102 (PASS 96/100), REC-103 (PASS 94/100), REC-104 (PASS 93/100 — fecha FND-03), REC-105 (PASS 97/100, concorrência real testada — fecha FND-12), REC-106 (PASS 95/100 — fecha FND-02 parte PII), REC-107 (PASS 96/100 — restaura inscrição pública, fecha a Onda 2), REC-201 (ADR-016, PASS/GO @po), REC-404 (PASS 92/100 — cobertura real 47%, sem meta inventada), REC-405 (PASS 91/100 — teste automatizado trava separação comparação/atualização), REC-407 (PASS 90/100 com waiver até 2026-08-16). **Onda 2 completa; parte da Onda 5 adiantada (REC-404, REC-405, REC-407).** Baseline constitucional verde (lint/typecheck/test:db 113/113/vitest 606/606). **REC-202 (Done, PASS 94/100)** — sessão Supabase SSR aditiva, HMAC intocado. **REC-203 (Done, PASS 95/100)** — mecanismo de resolução de papel no servidor construído como infraestrutura pura, testado (rebaixamento provado a nível de mecanismo), sem nenhuma rota real migrada — HMAC continua sendo a única autoridade real em produção. **REC-205 (Done, PASS 94/100)** — a chave de rate limit da inscrição pública passou a incluir, opcionalmente, um identificador (hash SHA-256, não-PII) da sessão SSR de REC-202, apenas para granularidade de bucket; requisições anônimas permanecem byte-idênticas a REC-107 (mesma chave `enrollment:<ip>`, mesmo limite, mesmas respostas), nenhuma decisão de autorização foi acoplada (REC-203 não consumida) e o auth HMAC de produção segue intocado. Suíte agregada 638/638. **REC-206 (Done, PASS 93/100 — inicia o fechamento de FND-08)** — o browser passou a ler leads administrativos por um único contrato same-origin (`admin-resources` `leads/list`, HMAC via `requireAdmin`, roteado por `/api/functions/[name]`); o contrato duplicado de leitura direta ao cliente Supabase (`fetchLeadsFromSupabase`) foi removido de `app-store.tsx` e `rh-cursos-api.ts`, com o refetch realtime também repontado para o BFF. Nenhuma autoridade nova (REC-203 não consumida; HMAC intocado); os read models completos de alunos/inscrições ficaram para REC-303/304 (agora habilitados); transporte realtime (WebSocket) e leituras públicas de catálogo permanecem diferidos/documentados. Suíte agregada 639/639 (+1). **REC-303 (Done, PASS pendente de @qa — fecha a parte alunos/inscrições de FND-08)** — criadas rotas same-origin NET-NEW (`app/api/admin/students`, `app/api/admin/enrollments`) que hidratam alunos/inscrições server-side após reload, com paginação (`page`/`pageSize`, cap 100) e filtros (turma, status, busca por nome/email sanitizada), lendo `inscricao` com `aluno`/`turma` embutidos via `createSupabaseServerClient()` (service-role) apenas após autorização. É a **PRIMEIRA ativação real de `requireServerRole()` (REC-203)**, porém EXCLUSIVAMENTE nessas rotas net-new — seguro porque não havia leitura server-side desse dado (per REC-206), logo sem comportamento existente para quebrar nem risco de lockout; **não** é o cutover de REC-204 (que troca a autoridade de rotas HMAC já em produção e segue proibido). Fail-closed provado por teste (sem sessão → 401 sem consultar o banco; papel insuficiente → 403; lockdown → 503; admin → 200 paginado). Limitação CONHECIDA e ACEITÁVEL documentada: a sessão SSR (REC-202) só é populada por `signInSSR`; o login admin de produção ainda é HMAC (`src/lib/auth.ts`), que não emite sessão SSR — logo as rotas hoje podem não ser consumíveis por um admin logado só via HMAC (integração fica para REC-305, não é bug). HMAC de produção intocado (`src/lib/auth.ts`, `supabase/functions/_shared/auth.ts`, `app/api/auth/session/route.ts`, `admin-resources/index.ts`). Suíte agregada 657/657 (+18). **REC-304 (Done, PASS pendente de @qa — continua o fechamento de FND-08, parte catálogo)** — estende o padrão de REC-303 ao núcleo do catálogo: rotas same-origin NET-NEW (`app/api/admin/courses`, `app/api/admin/classes`, `app/api/admin/instructors`) que hidratam cursos/turmas/instrutores server-side após reload, com paginação (`page`/`pageSize`, cap 100) e busca equivalente à UI (título/nome/título do curso, sanitizada), reutilizando as projeções DB→domínio já validadas (`mapCourse`/`mapClass`/`mapInstructor` de `mappers.ts` — nenhum mapper novo), com joins de página (`curso_instrutor`/`turma`) buscados scoped por `.in(...)`. Mesmo guard `requireAdminApi()` → `requireServerRole(ssrClient, "admin")` (REC-203) sobre a sessão SSR de REC-202; fail-closed provado por teste (sem sessão → 401 sem consultar o banco; student → 403; lockdown → 503; admin → 200 paginado). **Escopo deliberadamente reduzido (menor e sólido, lógica de REC-206):** leads, conteúdo (blog) e métricas ficaram como pendência explícita **REC-304b** (não criada formalmente) — leads já é servido por `admin-resources` `leads/list` (REC-206, duplicaria autoridade); métricas **não têm contrato server-side** (são derivadas no cliente a partir da store — implementar seria inventar contrato, viola Article IV). Mesma limitação CONHECIDA e ACEITÁVEL de REC-303 (admin logado só via HMAC recebe 401; integração fica para REC-305). HMAC de produção intocado (`src/lib/auth.ts`, `supabase/functions/_shared/auth.ts`, `app/api/auth/session/route.ts`, `admin-resources/index.ts`). Suíte agregada 672/672 (+15). **REC-306 (Done, gate PENDING aguardando @qa — fecha a parte de navegação/ações inertes de FND-17)** — escopo puramente frontend/UX no admin shell: a navegação móvel estava quebrada (o hambúrguer do `admin-topbar.tsx` alternava um estado `mobileOpened` que **não renderizava nada** e, na prática, o mobile só alcançava 5 dos 9 itens via barra inferior — Inscrições, Instrutores, Blog e Configurações eram inacessíveis no celular). Criado `admin-mobile-drawer.tsx` (Radix Dialog) que renderiza um drawer acessível com **todos** os itens de navegação + catálogo + logout, operável por teclado (Tab/Enter/Escape, foco preso e restaurado pelo Radix), com `aria-controls`/`aria-expanded` ligando o botão de toggle ao drawer. Controles inertes do topbar resolvidos caso a caso (FR-10): busca global inerte (sem handler, sem backing) **removida** — cada página de recurso já tem busca real; sinos de **Notificações** e **Ajuda** (sem sistema por trás, fora de escopo construir) **marcados explicitamente como indisponíveis** (`disabled` + `aria-disabled` + `title`/label "em breve"), sem implementação fake (Article IV). Nenhum arquivo de autenticação (HMAC/SSR) tocado. Suíte agregada 684 → 694 (+10 testes de menu móvel/ARIA/ações), lint/typecheck limpos. **REC-305 (Done, gate PENDING aguardando @qa — fecha FND-07)** — bugfix restrito de UX/frontend no login público, mantendo o HMAC como **único** mecanismo de sessão exatamente como hoje. A investigação confirmou que o **destino pós-login JÁ deriva do papel server-side**: `Login.tsx` redireciona por `data.session.role` (resposta do servidor), e o endpoint HMAC `app/api/auth/session/route.ts` é autoritativo — deriva o papel de `app_metadata.role` (route.ts:132) e responde **403** quando o portal pedido diverge do papel real (route.ts:137-139); `?next` é validado por `isRolePathAllowed` (sem open redirect). Nenhum bug de destino foi inventado (Article IV); a lacuna real era ausência de **trava de regressão** (`session-routing.ts` sem testes e sem teste de componente da derivação), agora fechada. O defeito **real** era o recovery: o botão "Esqueci minha senha" disparava `toast.success` **incondicional**, sem qualquer chamada de rede nem fluxo de reset por trás — falso sucesso (antipadrão de REC-302), violando Article IV/FR-10. Como recuperação de autosserviço real exigiria feature NET-NEW (página `reset-password` + token), fora do bugfix, seguindo "recovery é real **ou** removido" o controle enganoso foi **REMOVIDO** e o canal real ("Fale com a coordenação" → `/contato`) mantido. **HMAC intocado** (`src/lib/auth.ts`, `supabase/functions/_shared/auth.ts`, `app/api/auth/session/route.ts` sem modificação); nenhuma sessão SSR (REC-202) nem resolução de papel (REC-203) consumida pelo login; **isto NÃO é o cutover de REC-204** — nenhuma autoridade de login foi migrada. Suíte agregada 694 → 710 (+16), lint/typecheck limpos. **REC-308 (Done, gate PENDING aguardando @qa — fecha a parte de acessibilidade de FND-17, junto de REC-306)** — auditoria WCAG 2.1 AA (violações sérias/críticas) das jornadas críticas, escopo puramente frontend/acessibilidade, sem tocar auth. **Uma única violação SÉRIA** encontrada e corrigida: no formulário de pré-inscrição (`src/views/public/CourseCheckout.tsx`, componente `Field`) o erro de validação **não era associado** ao campo (sem `aria-describedby`, sem `aria-invalid`) e ficava dentro de um `<label>` implícito — leitor de tela não vinculava o erro ao input; corrigido com `id`/`htmlFor` explícitos + `cloneElement` injetando `aria-describedby`/`aria-invalid` e `id` no erro (`role="alert"` preservado). **Login (REC-305) e navegação admin (REC-306) auditados e confirmados JÁ conformes** — nenhuma correção inventada (Article IV); adicionadas travas de regressão de acessibilidade. **Catálogo (Courses/CourseDetail) sem violação séria/crítica**; duas pendências de baixa severidade documentadas (P-308-1 ícones decorativos sem `aria-hidden`; P-308-2 overlay `Play` decorativo). Medição instrumentada de contraste permanece via `npm run test:a11y` (@axe-core/playwright, browser real), fora do runner Vitest. HMAC/SSR intocados (`src/lib/auth.ts`, `supabase/functions/_shared/auth.ts`, `app/api/auth/session/route.ts`, `src/lib/supabase/session.ts`, `src/lib/supabase/authorize.ts`). Suíte agregada 710 → 715 (+5: 3 checkout + 2 login), lint/typecheck limpos. **REC-204 (Done, gate combinado @qa+@architect PASS com follow-ups não bloqueantes — fecha FND-04, desbloqueia REC-206/REC-502)** — cutover real de autoridade: HMAC (`src/lib/auth.ts`, `supabase/functions/_shared/auth.ts`, allowlist da Fase A) removido por completo do caminho de produção; login e `admin-resources` dependem exclusivamente da sessão SSR do Supabase (`requireServerRole`/`requireTrustedSsrAdmin`), forward-only (NFR-08, sem caminho de rollback para HMAC). Executado em duas fases com dois gates humanos distintos (Fase A em 2026-07-17, Fase B em 2026-07-18), risco de lockout mitigado por rollout em conta de teste isolada (projeto Supabase dedicado) antes do cutover total, com validação operacional real (login/logout/rebaixamento em rota HTTP real, não mock) antes de cada avanço. Achado HIGH da revisão de segurança (gap de `.gitignore` para backup local, sem segredo real exposto) remediado na mesma sessão. Follow-up MEDIUM não bloqueante registrado: subscriptions realtime do dashboard admin (`lead_changes`/`inscricao_changes`/`aluno_changes`) perderam autenticação (cliente browser agora permanentemente `anon`, RLS dessas tabelas exige `authenticated`) — decisão humana explícita de tratar como known-issue, a resolver junto da consolidação do transporte realtime já diferida por REC-206. Suíte agregada 715/715, lint/typecheck limpos. **REC-406 (Done — fecha a parte OpenAPI de FND-16)** — `docs/api/openapi.yaml` cobria só 6 de 13 rotas HTTP publicadas; adicionadas as 5 rotas admin (`GET /api/admin/*`), `ssr-session` e o BFF dinâmico `/api/functions/{name}` (segmento Next `[name]` normalizado pelo gate anti-drift). Documentação apenas — nenhum comportamento de rota alterado. `docs:api:check-drift` passa com 13 rotas reconciliadas; 15/15 testes de contrato novos; lint/typecheck/build verdes. **REC-408 (Done, gate combinado @architect+@qa PASS com follow-up não bloqueante — fecha FND-10)** — CSP consolidada em fonte única (`security-headers.ts`), removidas as declarações concorrentes de `next.config.mjs`/`public/_headers`; `cdn.jsdelivr.net` e `api.rhcursos.com.br` removidos do `connect-src` por não terem consumidor de código (AC2); `unsafe-eval` ausente em produção. `Cache-Control: no-store` aplicado por ponto único de saída nas 8 rotas sensíveis (auth, ssr-session, BFF, 5 admin), cobrindo sucesso/erro/MFA/logout/rate-limit. Redaction central recursiva no logger (nome de campo + formato de valor, `Error.message`/`cause`/`stack` saneados por ambiente) e telemetria de query saneada (categoria derivada de status/SQLSTATE, nunca da mensagem bruta do Postgres — fecha o vetor de vazamento de violação de constraint UNIQUE). Revisão combinada identificou inicialmente um FAIL por suspeita de cutover REC-204 embutido no changeset, mas investigação confirmou ser artefato de metodologia (`git diff HEAD` sem fronteira de commit entre as três stories da sessão) — corrigido commitando REC-204/REC-406/REC-408 separadamente. Suíte agregada 746/746, lint/typecheck/build verdes. **REC-005 (saneamento de histórico Git) segue igualmente pausada** — destrutiva/force-push. Pendência não bloqueante em REC-301: 16 baselines PNG regenerados em `tests/baseline/` aguardando decisão de commitar/descartar.
**Reconciliação de gates (2026-07-17):** `REC-303`, `REC-304`, `REC-305`, `REC-306` e `REC-308` estão formalmente `Done` com gate `PASS` de `@qa`. O gate REC-308 foi revalidado independentemente com lint/typecheck, 729/729 unitários e 9/9 Playwright/Axe WCAG 2.1 A/AA; P-308-1/P-308-2 permanecem follow-ups low não bloqueantes.
**Prioridade:** P0 / SEV-0
